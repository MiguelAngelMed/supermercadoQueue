/*
IMPLEMENTACION DE LA LIBRERIA DEL TAD COLA DINAMICA (TADColaDin.h)
AUTOR: LMSG 2021
VERSIÓN: 1.0
...
void Queue(cola * c, elemento e)
{
	...
	//Si la cola esta vacía, los apuntadores de la cola apuntarán al nuevo elemento
	if (c->num_elem==0)
	{
		c->frente = aux;
		c->final = aux;
...
elemento Dequeue(cola * c)
{
	...
	//Si la cola esta vacía (Subdesbordamiento de cola)
	if(c->num_elem==0)
	{
...
Descripción: Recibe una cola y TRUE si la cola está vacía y FALSE en caso contrario
...
Descripción: Recibe una cola y devuelve el elemento que se encuentra al frente de ésta.
Recibe: cola *c (Referencia a la cola "c")
Devuelve: elemento del frente de la cola
Observaciones: El usuario a creado una cola, la cola fue correctamente inicializada, ésta
tiene elementos de lo contrario devolverá ERROR. *No se modifica el TAD
*/
elemento Front(cola * c)
{
	return c->frente->e;
...
Descripción: Recibe una cola y devuelve el elemento que se encuentra al final de ésta.
Recibe: cola *c (Referencia a la cola "c")
...
/*
int Size(cola * c);
Descripción: Recibe una cola y devuelve el número de elemento que se encuentran en ésta.
Recibe: cola *c (Referencia a la cola "c")
...
Observaciones: El número i debera estar entre 1 y el tamaño de la cola, si está es vacía o mas pequeña se provoca un error.
*/
