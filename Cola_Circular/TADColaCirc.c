...
*DE MANERA CIRCULAR (Modificando las implementaciones al modelo circular)
AUTOR: LMSG 2021
VERSION: 1.0
...
Una cola circular es un modelo estático que eficientiza el proceso de desencolar
...
COMPILACION PARA GENERAR EL CODIGO OBJETO: gcc -c TADColaEstCir.c
*/
...
//DECLARACION DE FUNCIONES (AUXILIARES PROPIAS DE ESTE CODIGO NO SON PARA EL USUARIO DE LA COLA)
...
elemento Dequeue(cola * c)
{
	...
	{	
		...
		//Si se desencolo al último elemento
		if(c->num_elem==0)
			Initialize(c);
	}
...
{
	if (c->num_elem==0){
		printf("\nERROR: (Front) Se intenta acceder a elemento inexistente");
		exit(1);
	}
...
{
	if (c->num_elem==0){
		printf("\nERROR: (Final) Se intenta acceder a elemento inexistente");
		exit(1);
    }
...
Observaciones: MAX_ELEMENT determinar el incremento o reseteo del índice.
*/
int IncrementaIndice(int n)
{
	n++;
